...
import edu.indiana.d2i.flink.utils.PengProvState;
...
public class PengSerialGroupReducer extends ProcessFunction<Tuple2<String, ObjectNode>, ProvEdge> {

    private ValueState<PengProvState> state;
    ...
    public void open(Configuration parameters) throws Exception {
        state = getRuntimeContext().getState(new ValueStateDescriptor<>("local-state", PengProvState.class));
    ...
    public void processElement(Tuple2<String, ObjectNode> in, Context context, Collector<ProvEdge> out) throws Exception {
        PengProvState current = state.value();
        if (current == null) {
            current = new PengProvState();
            current.key = in.f0;
        }
